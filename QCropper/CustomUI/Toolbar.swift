...
class Toolbar: UIView {
    ...
    func titleButton(_ title: String, highlight: Bool = false) -> UIButton {
        ...
        if highlight {
            button.setTitleColor(QCropperConfig.highlightColor, for: .normal)
            button.setTitleColor(QCropperConfig.highlightColor.withAlphaComponent(0.7), for: .highlighted)
        } else {
